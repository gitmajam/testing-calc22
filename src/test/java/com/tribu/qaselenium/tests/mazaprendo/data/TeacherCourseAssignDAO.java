...
public class TeacherCourseAssignDAO {
	...
	private static final String SQL_SELECT = "SELECT username FROM " + DATA_BASE + ".assigned_courses WHERE username_teacher_course = ?";
	private static final String SQL_DELETE = "DELETE FROM " + DATA_BASE + ".assigned_courses WHERE username_teacher_course = ?";

	public Iterator<UserCourseAssign> select(TeacherCourseAssign person) {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		UserCourseAssign personFound = null;
		List<UserCourseAssign> persons = new ArrayList<>();
		try {
			...
			while (rs.next()) {
				...
				personFound = new UserCourseAssign(username);
	...
	public int remove(TeacherCourseAssign person) {
		Connection conn = null;
		PreparedStatement stmt = null;
		int registers = 0;
		try {

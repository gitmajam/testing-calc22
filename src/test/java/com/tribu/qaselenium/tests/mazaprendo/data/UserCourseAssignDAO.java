...
public class UserCourseAssignDAO {
	...
	private static final String SQL_SELECT = "SELECT username FROM " + DATA_BASE + ".assigned_courses_users WHERE username = ?";
	private static final String SQL_DELETE = "DELETE FROM " + DATA_BASE + ".assigned_courses_users WHERE username = ?";

	public Iterator<UserCourseAssign> select(UserCourseAssign person) {
	...
	public int remove(UserCourseAssign person) {
		Connection conn = null;
		PreparedStatement stmt = null;
		int registers = 0;
		try {
